...
export class Vertex {
  ...
  public showVertices = (object: THREE.Mesh) => {
    if (this.vertexHelpersMap.has(object)) return;
  ...
  public hideAllVertices = () => {
    this.vertexHelpersMap.forEach((helpers, mesh) => {
      if (helpers.parent) {
        helpers.parent.remove(helpers);
      }
    });
    this.vertexHelpersMap.clear();
    this.selectedVertices = [];
  }

  public hideVertices = (object: THREE.Mesh) => {
    const helpers = this.vertexHelpersMap.get(object);
    if (helpers && helpers.parent) {
      helpers.parent.remove(helpers);
      this.vertexHelpersMap.delete(object);
    }
  }
